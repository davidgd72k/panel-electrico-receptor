extends CharacterBody2D

const WALK_ANIM = "go_"
const PSY_SHOOT = preload("uid://deny88jv3l3bb")

enum ShootMode {
	WALK_MODE,
	AIM_MODE
}
@export var walk_speed: float = 100.0
@export var enter_direction: Vector2 = Vector2.DOWN
@export var acceleration: float = 10.0
@export var friction: float = 15.0
@export var shoot_mode: ShootMode = ShootMode.WALK_MODE

## Font used to display debug text.
var debug_font = preload("uid://crk02q7wwi7ou")
## Player look left_direction angle.
var current_look_angle: float
var left_direction: Vector2
var right_direction: Vector2

@onready var animation_player: AnimationPlayer = $AnimationPlayer
@onready var animation_tree: AnimationTree = $AnimationTree
@onready var state_machine: AnimationNodeStateMachinePlayback = $AnimationTree.get("parameters/playback")
@onready var ray_launcher: Node2D = $RayLauncher
@onready var psy_origin: Node2D = $RayLauncher/PsyOrigin
@onready var mode_label: Label = %ModeLabel

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	# Default animation look to south.
	update_blend(Vector2.DOWN)
	current_look_angle = Vector2.DOWN.angle()
	mode_label.text = "MODE: %s" % ShootMode.keys()[shoot_mode]


# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta: float) -> void:
	print("LOOK ANGLE: %f" % current_look_angle)
	if Input.is_action_just_pressed("use_power"):
		shoot_ray()
	
	# Change AIM Mode.
	if Input.is_action_just_pressed("switch_mode"):
		var index_enum = ShootMode.values()[shoot_mode]
		index_enum += 1
		shoot_mode = wrapi(index_enum, 0, len(ShootMode))
		mode_label.text = "MODE: %s" % ShootMode.keys()[shoot_mode]
		

func _physics_process(delta: float) -> void:
	left_direction = Input.get_vector("go_left", "go_right", "go_up", "go_down")
	right_direction = Input.get_vector("aim_left", "aim_right", "aim_up", "aim_down")
	var left_direction_norm = left_direction.normalized()
	var right_direction_norm = right_direction.normalized()
	
	if shoot_mode == ShootMode.AIM_MODE:
		if right_direction.length() != 0.0:
			current_look_angle = right_direction.angle()
	
	if Input.is_action_pressed("aiming"):
		if left_direction.length() != 0.0:
			if shoot_mode == ShootMode.WALK_MODE:
				current_look_angle = left_direction.angle()
		# Braking character when aiming.
		velocity.x = move_toward(velocity.x, 0.0, friction)
		velocity.y = move_toward(velocity.y, 0.0, friction)
	# Moving character.
	elif left_direction != Vector2.ZERO:
		# Rotate shoot arrow.
		if shoot_mode == ShootMode.WALK_MODE:
			current_look_angle = left_direction.angle()
		
		
		velocity.x = move_toward(velocity.x, left_direction_norm.x * walk_speed, acceleration)
		velocity.y = move_toward(velocity.y, left_direction_norm.y * walk_speed, acceleration)
		
	# Braking character.
	else:
		velocity.x = move_toward(velocity.x, 0.0, friction)
		velocity.y = move_toward(velocity.y, 0.0, friction)
	
	ray_launcher.rotation = current_look_angle
	# Update AnimationTree state for animating player sprite.
	update_blend(left_direction_norm)
	update_movement_state()
	move_and_slide()


func _draw() -> void:
	pass


func _input(event: InputEvent) -> void:
	pass


func _unhandled_input(event: InputEvent) -> void:
	pass


func update_blend(value) -> void:
	if value == Vector2.ZERO:
		return
	
	# Mirror Y value to match with BlendSpace2D look.
	value.y *= -1
	animation_tree.set("parameters/walk/blend_position", value)
	animation_tree.set("parameters/idle/blend_position", value)


func update_movement_state() -> void:
	if is_zero_approx(velocity.length()):
		state_machine.travel("idle")
	else:
		state_machine.travel("walk")

func shoot_ray() -> void:
	var shoot = PSY_SHOOT.instantiate()
	shoot.shoot_angle = current_look_angle
	var aim_distance = psy_origin.position
	shoot.global_position = self.global_position + aim_distance.rotated(current_look_angle)
	get_tree().current_scene.add_child(shoot)


func timeShoot(time: float):
	print("Tiempo parado en %d segundos" % time)


func _on_animation_player_current_animation_changed(name: StringName) -> void:
	pass
